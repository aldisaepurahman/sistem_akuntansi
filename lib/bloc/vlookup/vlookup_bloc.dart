...
class VLookupBloc extends Bloc<Event, VLookupState> {
  ...
  Future<void> _getAllCoaData(
      AkunFetched event, Emitter<VLookupState> emit) async {
    try {
      emit(state.copyWith(status: SystemStatus.loading));
      final list_coa = await service.getAllCOA(
          TableViewType.v_lookup.name, {"nama_akun": "%${event.keyword}%"});
      emit(state.copyWith(status: SystemStatus.success, list_coa: list_coa));
    } catch (_) {
