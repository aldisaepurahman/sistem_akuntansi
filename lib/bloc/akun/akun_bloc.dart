...
class AkunBloc extends Bloc<Event, AkunState> {
  ...
  Future<void> _insertData(AkunInserted event, Emitter<AkunState> emit) async {
    try {
      ...
      await service.insert(TableViewType.saldo_coa.name, event.saldo.toJson());
  ...
  Future<void> _updateData(AkunUpdated event, Emitter<AkunState> emit) async {
    try {
      ...
      await service.update(TableViewType.coa.name, event.akun.toJson(),
          {"kode_akun": event.akun.kode_akun});
      await service.update(TableViewType.saldo_coa.name, event.saldo.toJson(),
          {"id_saldo": event.saldo.id_saldo});
      emit(state.copyWith(status: SystemStatus.success));
    } catch (_) {
      emit(state.copyWith(status: SystemStatus.failure));
    }
  ...
  Future<void> _deleteData(AkunDeleted event, Emitter<AkunState> emit) async {
    try {
      ...
      await service
          .delete(TableViewType.saldo_coa.name, {"kode_akun": event.kode_akun});
      emit(state.copyWith(status: SystemStatus.success));
    } catch (_) {
      emit(state.copyWith(status: SystemStatus.failure));
    }
